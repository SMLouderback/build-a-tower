...
- Fallback order: active star → nearest lower star → nearest higher star → `lobby_pan_{PP}` → `lobby_mid_*` / procedural.
...
- [ ] **Step 2: Run EditMode tests, expect failures** (methods missing).

- [ ] **Step 3: Implement selection + load**
...
- [ ] **Step 4: Rebuild cache on star change**

In `EnsureLoaded`, set `if (_lobbyStar < 0) _lobbyStar = LobbyStarSets - 1;` and build pans via `LoadLobbyPanForStar(_lobbyStar, p)`. Extract the pan-building loop into `static void BuildLobbyPanTiles()` so it can rerun.
...
- [ ] **Step 1: Add a lobby repaint entry point** on `TilemapTowerView` that repaints all lobby rooms using the existing lobby paint path (reuse `RepaintAllRooms` if it already covers structure tiles; otherwise iterate rooms where `room.Type.isLobby` and call the existing lobby paint method).
- [ ] **Step 2: Call it from `BuildController.RefreshStairsArt`** so a star change refreshes stairs overlays **and** lobby tiles. Keep the existing method name working (rename with the old name delegating, or update the single `TowerSimulation` call site).
- [ ] **Step 3: Verify compile** (no other callers broken).

### Task 3: Art — rename 5★ set, generate 0★–4★

**Files:** `Assets/Resources/Art/Structure/lobby_s{SS}_pan_{PP}.{png,bytes}` (+ `.meta`)

- [ ] **Step 1: Rename** `lobby_pan_01..05` → `lobby_s05_pan_01..05` (png, bytes, metas).
- [ ] **Step 2: Generate 5 wide images per tier** for 0★–4★ using the moods in the spec table.
- [ ] **Step 3: Normalize** each to 640×128 (plate/letterbox crop, 5:1 center crop, force opaque), write `.png` + `.bytes`, create metas from an existing template.
- [ ] **Step 4: Play Mode check** — set stars low/high and confirm the lobby swaps, no white bars, no pink.

### Task 4: Docs

- [ ] Mark spec **Implemented**; note star-tier lobby art in the lobby panorama spec follow-ups.
